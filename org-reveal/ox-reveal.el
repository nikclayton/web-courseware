...
'>
          ...
          (format
            ...
            (if use-highlight
              ...
              (format "\n<pre %s%s>%s</pre>"
                      (or (frag-class frag info)
			  (let ((at-classes `("src" ,(format "src-%s" lang)))
				(at-style "")
				(at-lang (format "data-ox-src-language=\"%s\"" lang)))
			    (when num-start
			      (add-to-list 'at-classes "line-number")
			      (setq at-style (format "style=\"--data-ox-src-line-start: %s\"" num-start)))
			    (format
			     " class=\"%s\" %s %s"
			     (string-join at-classes " ")
			     at-style
			     at-lang)))
                      ;(format " class=\"src src-%s\" style=\"--data-ox-src-line-start: %s\" data-ox-src-language=\"%s\"" lang (or num-start 0) 1) lang))
                      label code)
              )))))))
...
contextual information."
  (let* ((frag (org-export-read-attribute :attr_reveal quote-block :frag))
	 (frag-class (when frag
			 (frag-style frag info)))
	 (other-class (org-export-read-attribute :attr_reveal quote-block :class)))
    (format "<blockquote class=\"%s\">\n%s</blockquote>"
	    (string-join `(,frag-class ,other-class) " ") contents)))
