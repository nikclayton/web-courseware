...
- Four possible solutions
  - In increasing level of quality
...
However, if you actually do this it won't work as witten on this slide.

This is because the =onKeyup= function needs access to all the variables defined at the start of =domContentLoaded()=. Since =onKeyup= is it's own function this doesn't work.
#+END_NOTES

**** Solution 2: With variables

#+NAME: solution-with-variables
#+BEGIN_SRC javascript
function onKeyup(event) {
...
#+BEGIN_NOTES
So we could duplicate all the variable declarations at the start of =domContentLoaded()= and put them at the start of =onKeyup()= as well.

This works. But again, it's code duplication, which we want to avoid.

Lets put =onKeyup= somewhere it can see all those variables.

But first, a refresher on scope.
#+END_NOTES
...
#+BEGIN_NOTES
Here's our problem again -- if we put our =onKeyup= function outside the =domContentLoaded= function then we need to duplicate the variables that =domContentLoaded= has.

But based on what we've just learned about scopes, we can put the =onKeyup= function *inside* the =domContentLoaded= function. It will have access to all the variables inside =domContentLoaded= (because it will create a child scope).

In addition, =onKeyup= only needs to be called from =domContentLoaded=. So if we put it inside that function then it will only be visible there, meaning there are fewer things cluttering up the global scope.
#+END_NOTES

** Solution 3: =onKeyup= inside =domContentLoaded=
...
- Creates the =items_= array, and =view_=
- =src/js/model.js=
...
- Model:
  ...
  - Create a method to retrieve the =items_= array
    - Prevent changes to the =items_= array
    - Make the method a get'er
- View:
...
*** Getter turns a method in to a property
...
#+END_NOTES
*** Exercise: =View.update()=

- Find the shopping list element (=ul=)
- Remove all its children
- Iterate over the =items=
  - Convert it to a listitem (=li=, use the =toListItem= method)
  - Append it as a child to the list

*** Solution: =View.update()=

#+INCLUDE: "shopping-mvc/dummy-data/src/js/view.js" :lines "9-19" src javascript

#+REVEAL: split

#+INCLUDE: "shopping-mvc/dummy-data/src/js/view.js" :lines "20-34" src javascript
...
#+INCLUDE: "shopping-mvc/add-item-button/src/js/view.js" :lines "9-32" src javascript

#+REVEAL: split

#+INCLUDE: "shopping-mvc/add-item-button/src/js/view.js" :lines "31-40" src javascript

#+REVEAL: split

#+INCLUDE: "shopping-mvc/add-item-button/src/js/view.js" :lines "44-59" src javascript

*** =Controller=

#+INCLUDE: "shopping-mvc/add-item-button/src/js/controller.js" src javascript

*** =Model=

#+INCLUDE: "shopping-mvc/add-item-button/src/js/model.js" :lines "25-34" src javascript
...
#+INCLUDE: "shopping-mvc/delete-item-button/src/js/view.js" :lines "45-61" src javascript
