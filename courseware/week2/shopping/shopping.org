...
*** What if you wanted to process every second element?
 ...
 for (const i = i; i < a.length; i = i + 2) {
   console.log(a[i]);
 }
 #+END_SRC
...
#+BEGIN_NOTES
Here, the value of the =name= property is another object, with
...
#+BEGIN_NOTES
Like arrays, but instead of using a number as the index you use
a string.
#+END_NOTES
...
#+BEGIN_NOTES
For example, this is useful when the name of the property you
want to access is stored in a variable.
#+END_NOTES
...
This *does* *not* mean that we can't change the value of its
properties -- we can.

It does mean that after we have declared =person= to contain
one object, we can *not* assign it a completely different object.
#+END_NOTES
...
#+BEGIN_NOTES
This way of writing complex data structures has become popular in
the industry, and you will see it referred to as JSON, or Javascript
Object Notation.
#+END_NOTES
...
#+BEGIN_NOTES
Notice how the code in the =greet= function uses the =this=
keyword.

Code inside an object can refer to other properties in the same
object. To do so it uses the special keyword =this=.

So in the code here, =this.name= means "The value of the name
...
#+BEGIN_NOTES
If we change the value of the property then accessing it via
=this= in the code ensures that we always get the correct value.

In the example here the =greet()= function uses it to ensure that
when the =name.first= property is changed it uses the correct
value.
#+END_NOTES
...
#+BEGIN_NOTES
So we can have a single variable with an arbitrarily complex
structure.
#+END_NOTES
...
#+BEGIN_NOTES
This is shopping-20.js and onwards.

Changest to cover in shopping-20.js

Order: 2
- JSDoc update, line 4
- Param name update, line 7
- item.name, line 11
- item.quantity, line 27, 30

Order: 3
- Create item, line 53-56
- Pass item, line 58

Order: 1
- Create item, line 78-81
- Pass item, line 83
...
This is no different than the ways we've been creating strings or
numbers -- we write the literal value that we want the variable
to have.

However, this can get tiresome when we have objects with a
complex structure. It's more typing (imagine we had to create
lots of different objects like this) and there's plenty of
opportunity to make mistakes -- for example, by accidentally
misspelling the name of a property.
...
- *Constuctor* is any function that creates ("constructs") an
  object
...
#+BEGIN_NOTES
When we create an object, the technical term for it is *constructing*
the object.
...
let nik = new Person('Nik', 'Clayton', ['Teaching', 'Skiing']);

nik.first.name;   // 'Nik'
nik.greet();      // 'Hello Nik'

let chris = new Person('Christian', 'Hirsig', ['Powercoders']);
chris.interests;  // ['Powercoders']
chris.greet();    // 'Hello Chris'
#+END_SRC

** Using a constructor in our code

#+INCLUDE: "shopping-21.js" :lines "39-50" src javascript

#+BEGIN_NOTES
JSDoc, @constuctor tag.

Remember, =this=, plus no =return= value.

shopping-21.js

Show the constructor function, + JSDoc

Using it, line 65 and 87

Change to createNewListItem JSDoc.
#+END_NOTES

** Using a constructor in our code

#+INCLUDE: "shopping-21.js" :lines "62-72" src javascript


** Using a constructor in our code

#+INCLUDE: "shopping-21.js" :lines "79-94" src javascript
...
So a more concise way to write this code would be create the
new =ShoppingListItem= object when we call =createNewListItem=,
like this.

As we've done before, you read this inside-out to understand
what's going on.
#+END_NOTES
...
#+BEGIN_NOTES
Everything that I've just shown you is the old-style way of creating
and referring to objects.

Normally in this course we've been focused on the most modern way
of writing code, and haven't paid too much attention to older ways
of doing so.

Objects are different, for two main reasons.

The first is that there is a vast amount of code out there that uses
the old way of doing things, and you will definitely see it in your
jobs. So you need to be able to understand and work with it.

The second is that this way of writing objects, using JSON, is
*extremely* popular, even outside Javascript, so it's important
that you get used to it.
#+END_NOTES

* New-style object syntax: Goal

#+BEGIN_SRC javascript
const itemInstance = new ShoppingListItem('Eggs', '6');
itemInstance.name;
  // Property, "Eggs"
itemInstance.quantity;
   // Property, "6"
itemInstance.asListItem();
  // Method
  // <li><span>Eggs</span> <span>(6)</li> <button>delete</button>
#+END_SRC

#+BEGIN_NOTES
This is what we're working towards.

The code that we're writing will let us (eventually), write this.

Our object's class is going to be called =ShoppingListItem=. Note that
this is the UpperCamelCase -- it's very common to use that for class
names.

Our object is going to have two properties, one for the item name, and
one for the quantity.

To create an *instance* of the object we use the =new= keyword, and the
name of the object. We pass in two parameters, one for the name of the
object, and one for the quantity.

Our object is going to have two properties that let us retrieve the item's
name and quantity. And it will have one method, that generates the HTML
elements to show the item in a list.
...
- Object is described by a *class*
 - Defines what the object is
 - A bit like a blueprint, or general instructions
- After defining an object you create an *instance* of it
- Objects have
...
#+BEGIN_SRC javascript
let item1 = new ShoppingListItem('Eggs', '6');
let item2 = new ShoppingListItem('Flour', '500g');
...
#+BEGIN_NOTES
We can create multiple instances of the same object, with different values,
and assign them to different variables.

Each one of these is a different *instance* of the object.

These are kept completely separate. The values in one don't affect the
values in the other.
#+END_NOTES
...
All the code that makes up the class will go inside those curly brackets.

This helps keep all the code together.

For example, in the current version of our code the constructor
function for the =ShoppingListItem= is in with all the other code,
making it difficult to see the relationships.
#+END_NOTES

** New style constructors: Making =new= work

#+BEGIN_SRC javascript
let itemInstance = new ShoppingListItem('Eggs', '6');
#+END_SRC
...
#+BEGIN_NOTES
This is what a new-style constructor looks like.

There are some similarities and differences to what we've already
seen.
...
[Text below is old version, and can be moved to where old-style
constructors are first introduced]
When we use the =new= keyword we say we are *constructing* the object.

We have to tell Javascript how our object should be constructed, and we
use a function for that.

This function *must* have the name =constructor=.

It can take parameters just like a function does. Our constructor needs
to take two parameters, one for the name, one for the quantity, so we
list them in parentheses, just like we do for functions.

Inside the body of the constructor things are a little different. We
want to save the values that were passed in.

In order to do that we use the special variable =this=. What's that?

Rememeber, the code in our class is the blueprint for all *instances* of
the object. We need some way to refer to the variables for this instance,
and to do that we use the keyword =this=.

So this code is saying -- construct a new instance of this object, and set
this instance's name and quantity appropriately.

*Important*: Notice how we do not use the =function= keyword here.
#+END_NOTES
...
#+BEGIN_NOTES
This is enough code to get this working.
#+END_NOTES

** Implementing =toListItem()=
