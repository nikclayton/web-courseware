...
#+PROPERTY: header-args:sql-mode+ :results verbatim
...
#+BEGIN_SRC sql-mode
SELECT name, milliseconds FROM track LIMIT 10
#+END_SRC
...
#+BEGIN_NOTES
We can think of our data as moving through a number of stages as our query is executed.

Unfortunately, these stages don't occur in the same order as we write them in the query.

The first stage consists of gathering all the data. This is the =FROM= clause in the query.

In this diagram this is the first table. The =track= table is represented here as a number of different columns. The arrows show how the data flows.

The second stage (so far) is the =SELECT= clause in the query. This receives the data from the =FROM= clause, and chooses the columns that are allowed through.

The result of the =SELECT= clause is -- generally -- a narrower table.

In this example we can see that that's true -- the table from the =FROM= clause has more than 4 columns. However, the table coming out of the =SELECT= clause has only two columns.
#+END_NOTES
...
#+BEGIN_SRC sql-mode
SELECT UPPER(name),
       CAST(milliseconds AS FLOAT) / 1000
  FROM track
 WHERE name LIKE '%stairway%'
   AND milliseconds > 500000;
#+END_SRC
...
#+NAME: group-by-composer
 ...
 #+REVEAL_HTML: <div class="rightcol">
#+RESULTS: group-by-composer
#+begin_example
mean_duration     Composer
----------------  ----------
711.493507157464
235.179666666667  A. F. Iomm
276.871           A. Jamal
397.531           A.Bouchard
206.994           A.Isbell/A
306.657375        AC/DC
...
*** Incorrect: Average time by composer

#+REVEAL_HTML: <div class="leftcol">
#+NAME: group-by-composer-wrong
#+BEGIN_SRC sql-mode
  SELECT AVG(milliseconds / 1000) AS mean_duration,
         composer
    FROM track
...
#+REVEAL_HTML: <div class="rightcol">
#+RESULTS: group-by-composer-wrong
#+begin_example
mean_duration    Composer
---------------  ----------
711.00408997955
...
#+BEGIN_NOTES
This is incorrect because it divides an integer by an integer (creating another integer result) and then computes the average of that. This means that the data is subtly wrong.
#+END_NOTES
...
#+NAME: count-time
#+BEGIN_SRC sql-mode
SELECT SUM(milliseconds) / 1000.0 as total_seconds
  FROM track;
#+END_SRC
#+REVEAL_HTML: </div>
...
#+REVEAL_HTML: <div class="rightcol">
#+RESULTS: count-time-by-composer
#+begin_example
total_seconds  Composer
-------------  ----------
695840.65
27217.126      Steve Harr
11271.816      U2
8996.378       Miles Davi
8667.178       Jagger/Ric
7912.553       Billy Corg
...
*** Solution: Shortest songs                                       :noexport:
...
In this example, you get exactly the same results, so =ORDER BY= is certainly simpler.

The reason to use =MIN()= is when you want to find the minimum value in some group of things. For example, the next exercise...
#+END_NOTES

*** Exercise: Shortest song written by each composer

- What's the shortest song written by each composer?

*** Solution: Shortest song written by each composer

#+REVEAL_HTML: <div class="leftcol">
#+NAME: min-track-by-composer
#+BEGIN_SRC sql-mode
  SELECT MIN(milliseconds),
         name,
         composer
...
#+REVEAL_HTML: <div class="rightcol">
#+RESULTS: min-track-by-composer
#+begin_example
MIN(milliseconds)  Name                      Composer
-----------------  ------------------------  -----------
1071               É Uma Partida De Futebol  Samuel Rosa
4884               Now Sports
7941               Commercial 1              L. Muggerud
32287              Casinha Feliz             Gilberto Gi
33149              Mateus Enter              Chico Scien
34168              Homem Primata (Vinheta)   Titãs
38164              Freedom For My People     Mabins, Mac
39131              Demorou!                  Mônica Mari
42240              Little Guitars (Intro)    Alex Van Ha
43232              The Star Spangled Banner  Hendrix, Ji
#+end_example
#+REVEAL_HTML: </div>

*** Exercise: Longest songs                                        :noexport:

- What are the names of the 10 longest songs, and who wrote them?

*** Solution: Longest songs                                        :noexport:
...
#+REVEAL_HTML: <div class="leftcol">
#+NAME: avg-song-per-album
#+BEGIN_SRC sql-mode
   SELECT AVG(track.milliseconds) / 1000.0 AS sec,
          album.title
     FROM track
LEFT JOIN album ON track.albumid = album.albumid
...
#+REVEAL_HTML: <div class="rightcol">
#+RESULTS: avg-song-per-album
#+begin_example
sec               Title
----------------  ----------------------
436.554777777778  ...And Justice For All
287.370166666667  20th Century Masters -
198.064           A Copland Celebration,
432.294454545455  A Matter of Life and D
298.931833333333  A Real Dead One
321.635818181818  A Real Live One
338.243           A Soprano Inspired
268.167266666667  A TempestadeTempestade
279.531117647059  A-Sides
182.365533333333  Ace Of Spades
#+end_example
#+REVEAL_HTML: </div>

** Shortest song on an album

- Show the shortest song on each album
- 3 columns in the output
  - Song duration, in seconds
  - Song name
  - Album name
- Albums wth shortest songs first
#+REVEAL: split

#+REVEAL_HTML: <div class="leftcol">
#+NAME: shortest-song-per-album
#+BEGIN_SRC sql-mode
     SELECT MIN(track.milliseconds) / 1000.0 AS sec,
	    track.name,
	    album.title
       FROM track
  LEFT JOIN album ON track.albumid = album.albumid
...
#+REVEAL_HTML: <div class="rightcol">
#+RESULTS: shortest-song-per-album
#+begin_example
sec         Name                      Title
----------  ------------------------  --------------
1.071       É Uma Partida De Futebol  O Samba Poconé
4.884       Now Sports                Body Count
7.941       Commercial 1              House of Pain
29.048      Bossa                     Os Cães Ladram
32.287      Casinha Feliz             As Canções de
33.149      Mateus Enter              Afrociberdelia
33.619      Deixa Entrar              Deixa Entrar
34.168      Homem Primata (Vinheta)   Acústico
38.164      Freedom For My People     Rattle And Hum
39.131      Demorou!                  Demorou...
#+end_example
#+REVEAL_HTML: </div>
** Longest song on an album

- Same thing, but now I want the longest song

#+REVEAL: split

#+REVEAL_HTML: <div class="leftcol">
#+NAME: longest-song-per-album
#+BEGIN_SRC sql-mode
     SELECT MAX(track.milliseconds) / 1000 AS sec,
	    track.name,
	    album.title
       FROM track
  LEFT JOIN album ON track.albumid = album.albumid
...
#+REVEAL_HTML: <div class="rightcol">
#+RESULTS: longest-song-per-album
#+begin_example
sec         Name                    Title
----------  ----------------------  ------------------------------
5286        Occupation / Precipice  Battlestar Galactica, Season 3
5088        Through a Looking Glas  Lost, Season 3
2960        Greetings from Earth,   Battlestar Galactica (Classic)
2825        Dave                    Lost, Season 2
2687        How to Stop an Explodi  Heroes, Season 1
2631        The Moth                Lost, Season 1
2622        Battlestar Galactica:   Battlestar Galactica: The Stor
2612        Something Nice Back Ho  LOST, Season 4
2541        The Job                 The Office, Season 3
2484        Pilot                   Aquaman
#+end_example
#+REVEAL_HTML: </div>
...
- 2 columns in the output
  - A count of the number of songs
  - The album name
- Albums with most tracks first

#+REVEAL: split

#+REVEAL_HTML: <div class="leftcol">
#+NAME: tracks-per-album
#+BEGIN_SRC sql-mode
...
** How many tracks in each genre                                   :noexport:
...
** Which album has the largest number of genres                    :noexport:

*** Start with

#+NAME: most-genres-1
#+BEGIN_SRC sql-mode :results verbatim :exports both
     SELECT genre.name,
	    album.albumid,
	    album.title
       FROM track
...
#+RESULTS: most-genres-1
#+begin_example
Name        AlbumId     Title
----------  ----------  -------------------------------------
Rock        1           For Those About To Rock We Salute You
Rock        2           Balls to the Wall
Rock        3           Restless and Wild
Rock        3           Restless and Wild
Rock        3           Restless and Wild
Rock        1           For Those About To Rock We Salute You
Rock        1           For Those About To Rock We Salute You
...
#+RESULTS: most-genres-2
#+begin_example
cnt         Name        AlbumId     Title
----------  ----------  ----------  ----------------------
9           Metal       156         ...And Justice For All
12          Rock        257         20th Century Masters -
1           Classical   296         A Copland Celebration,
11          Rock        94          A Matter of Life and D
12          Metal       95          A Real Dead One
11          Metal       96          A Real Live One
1           Classical   285         A Soprano Inspired
15          Latin       139         A TempestadeTempestade
17          Rock        203         A-Sides
15          Metal       160         Ace Of Spades
#+end_example
