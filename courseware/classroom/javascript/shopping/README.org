#+Title: DOM Shopping List Exercise
...
#+BEGIN_CENTER
[[file:samples/initial-wireframe.png]]
#+END_CENTER
...
#+INCLUDE: "samples/shopping-1.html" src html
...
#+REVEAL_HTML: <div class="leftcol">
#+INCLUDE: "samples/shopping-2.html" :lines "12-18" src html -n 11
...
*** Solution: Add a =button= element
  ...
  #+REVEAL_HTML: <div class="leftcol">
  #+INCLUDE: "samples/shopping-3.html" :lines "11-19" src html -n 11
  #+REVEAL_HTML: </div>
...
We see that this has a child element called =span=, so we add that to the tree.

Then we see that it has some content, so we add that as well.

Then the =span= ends, and there's a space.

After the space a new =button= element starts. This is also a child of the =li= element. And that has some text content too, so we add that as its own node.
#+END_NOTES
...
#+BEGIN_SRC dot :file list-item-graph4.svg :cmdline -Tsvg -Gstylesheet=../../graphviz.css
 digraph G {
   ...
   space2 [label="' '" style=invis shape=none];
...
#+INCLUDE: "samples/shopping-3.js" :lines "26-43" src javascript
...
#+BEGIN_SRC javascript
  ...
  console.log(a + ' ' + b + ' ' + c); // "Hello Powercoders students"
  console.log(`${a} ${b} ${c}`);      // "Hello Powercoders students"
#+END_SRC
...
#+REVEAL_HTML: <div class="rightcol">
#+BEGIN_SRC dot :file scope-global.svg :cmdline -Tsvg -Gstylesheet=../../graphviz.css
  digraph G{
    node [shape=record, fontname="Consolas" fontsize=16];
    global [label="{Global scope|let a = 10;|Name : Value\n - a \ \ : 10\l - foo : function() \{ ... \}\l- bar : function() \{ ... \}\l}"];
}
#+END_SRC

#+RESULTS:
[[file:scope-global.svg]]

#+REVEAL_HTML: </div>

#+BEGIN_NOTES
In the diagram on the right each box has three sections.

The first section is the name of the scope. In this case, this is the global scope.

The second section is the code that's run.

The third section shows the names of things that Javascript knows about at this point, and the value that that thing has.

In this case, Javascript knows that there is a thing called =a= that exists, and it has the value =10=.

Javascript also knows that there are two other things that exist. These have the names =foo=, and =bar=, and they are both functions.

Remember, functions are values too.

This third section -- the space that contains the names -- the technical term for that is a *namespace*.
#+END_NOTES
** Function scope
...
#+REVEAL_HTML: <div class="rightcol">
#+BEGIN_SRC dot :file scope-function-1.svg :cmdline -Tsvg -Gstylesheet=../../graphviz.css
  digraph G{
    node [shape=record, fontname="Consolas" fontsize=16];
    global [label="{Global scope|let a = 10;|Name : Value\n - a \ \ : 10\l - foo : function() \{ ... \}\l- bar : function() \{ ... \}\l}"];

    global -> foo;
    foo [label="{Function scope|<b>function foo() \{\l&nbsp;&nbsp;let b = 20;\l&nbsp;&nbsp;console.log(`$\{a\} $\{b\}`);\l\}\l|Name : Value\nb : 20}"];
  }
#+END_SRC

#+RESULTS:
[[file:scope-function-1.svg]]

#+REVEAL_HTML: </div>

#+BEGIN_NOTES
This is what happens when we run the =foo= function.

We type =foo()= in to the console, and press return. In the console, any code we type in is running in the global scope. Javascript looks in the global scope to see if a thing called =foo= exists, and what its value is.

=foo= does exist in the global scope, and is a function. Because we used the =()= after the name Javascript knows to call the function in =foo=, so it does, and the function starts.

The function creates a new scope, which is a child of the global scope.

This scope contains one extra thing, =b=, which has the value =20=.

When the =console.log= statement executes, Javascript has to print the values of =a= and =b=.

Javascript starts by trying to find =a= in the current scope -- the function scope.  As you can see, it's not there.

So Javascript looks in the parent scope for =a=. It finds it, and its value, so it can print =10=.

Then it has to find the thing called =b=. Again, it starts looking in the function scope. It finds it, so it doesn't have to look anywhere else.
#+END_NOTES
** Function scope
...
#+REVEAL_HTML: <div class="rightcol">
#+BEGIN_SRC dot :file scope-function.svg :cmdline -Tsvg -Gstylesheet=../../graphviz.css
  digraph G{
    node [shape=record, fontname="Consolas" fontsize=16];
    global [label="{Global scope|let a = 10;|Name : Value\n - a \ \ : 10\l - foo : function() \{ ... \}\l- bar : function() \{ ... \}\l}"];

  global -> bar;
  bar [label="{Function scope|function bar(msg) \{\l&nbsp;&nbsp;let b = 15;\l\l&nbsp;&nbsp;for (...) \{ ... \}\l\l&nbsp;&nbsp;if (true) \{... \}\l\}\l|Name : Value\n - msg: 'hello'\l - b : 15\l}"];
}
#+END_SRC

#+RESULTS:
[[file:scope-function.svg]]

#+REVEAL_HTML: </div>

#+BEGIN_NOTES
This is what happens if we type =bar()= in to the console.

Again, Javascript has to find out if =bar= exists. In the console we're in the global scope, so it checks, and =bar= does exist, and is a function, so Javascript calls it.

The =bar= function starts running, and creates its own scope, just like =foo= did.

Importantly, the =foo= scope no longer exists -- it disappears when the =foo= function stops running. This is why the =bar= function can't see any variables from =foo=, they don't exist at this point.

In the scope for =bar= there are two things. The first is the parameter value -- =bar= takes a parameter called =msg=, and when we called the function we gave it a string value of 'hello'.

This is why the things in this section of the diagram are not just variables -- any value we can give a name to goes in here -- variables, constants, functions, and parameters.

The second thing in here is the variable =b=, which has the value =15.
#+END_NOTES
...
#+REVEAL_HTML: <div class="rightcol">
#+BEGIN_SRC dot :file block-scope.svg :cmdline -Tsvg -Gstylesheet=../../graphviz.css
  digraph G{
    node [shape=record, fontname="Consolas" fontsize=16];
    ...
    bar -> for;
    for [label="{Block scope|for (let i = 0; i \< 3; i++) \{\l&nbsp;&nbsp;console.log(i);\l\}\l|Name : Value\n - i : 0..3\l}"];
  }
#+END_SRC

#+RESULTS:
[[file:block-scope.svg]]
#+REVEAL_HTML: </div>

#+BEGIN_NOTES
When the =for= loop starts running it creates its own scope, a block scope.

In here, any variables we define either in the block, or in the parentheses, are visible. We only have one, the variable =i=, which is why we can log the value.
#+END_NOTES
** Block scope
...
#+REVEAL_HTML: <div class="rightcol">
#+BEGIN_SRC dot :file scope.svg :cmdline -Tsvg -Gstylesheet=../../graphviz.css
  digraph G{
    node [shape=record, fontname="Consolas" fontsize=16];
    ...
    bar -> if;    if [label="{Block scope|if (true) \{\l&nbsp;&nbsp;let x = 10;\l&nbsp;&nbsp;console.log(`$\{a\} $\{b\} $\{x\} $\{msg\}`);\l\}\l|Name : Value\n- x : 10\l}"];
  }
#+END_SRC
...
#+BEGIN_NOTES
Now the =if= block starts running.

The scope from the =for= loop has disappeared. This is why the =i= variable in the loop is no longer accessible -- the scope it was in no longer exists.

The =if= block creates its own scope, and creates a new variable in that scope, =x=.

Then it logs the string.

To find the value of =a=, Javascript starts looking in the block scope. There's nothing in that scope with the name =a=, so it moves up one scope, looking in the function scope. There's nothing with that name there either, so it moves up to the global scope, and finds it.

It does the same thing with =b=, finding it one scope up (in the function scope).

=x= is in the current scope. And =msg= is the parameter passed to the function, so is found in the function scope.
#+END_NOTES
...
#+REVEAL_HTML: <div class="rightcol">
#+BEGIN_SRC dot :file scope-all.svg :cmdline -Tsvg -Gstylesheet=../../graphviz.css
  digraph G{
    node [shape=record, fontname="Consolas" fontsize=16];
...
- We can put a function inside another function
- The inner function
  - Has access to the variables in the outer function
  - Can *only* be called by the outer function

** Functions inside functions

#+REVEAL_HTML: <div class="leftcol">
#+NAME: scope-function-js
#+BEGIN_SRC javascript
  let a = 10;
...
#+REVEAL_HTML: <div class="rightcol">
#+BEGIN_SRC dot :file scope-func-in-func.svg :cmdline -Tsvg -Gstylesheet=../../graphviz.css
  digraph G{
    node [shape=record, fontname="Consolas" fontsize=16];
    global [label="{Global scope|let a = 10;|Name : Value\n- a \ \ : 10\l- foo : function() \{ ... \}\l}"];

    global -> foo;
    foo [label="{Function scope|function foo() \{\l\ \ let b = 15;\l\l\ \ function bar() \{\l\ \ \ \ console.log(`$\{a\} $\{b\}`);\l\ \ \}\l\l\ \ bar();\l\}\l|Name : Value\n \ - bar : function() \{ ... \}\l \ - b \ \ : 15\l}"];

  foo -> bar;
  bar [label="{Function scope|function bar() \{\l\ \ console.log(`$\{a\} $\{b\}`);\l\}\l|Name : Value}"];
  }
#+END_SRC

#+RESULTS:
[[file:scope-func-in-func.svg]]

#+BEGIN_NOTES
Here's different code from earlier. Our =foo= function now also contains a =bar= function.

Here are the scopes when we call =foo=.

As before, the global scope contains the names =a= and =foo=, where =a= has the value =10= and =foo= contains a function.

When we call =foo= it creates a function scope that contains =b= and =bar=. When -- at the bottom of the =foo= function -- it calls the =bar= function this works, because it can see the name.

The =bar= function creates its own scope, but does not create any new names of its own. The =console.log= line that references the names =a= and =b= works because =b= is in the parent scope (the function scope) and =a= is in the grandparent scope (the global scope).

However -- the =bar= function is *not* visible from the global scope. The global scope can only see the names =a= and =foo=. This is why, if we try and call the =bar= function ourselves, we can't, it won't work.

We have effectively hidden the =bar= function from everything except the =foo= function.
#+END_NOTES
...
#+INCLUDE: "samples/shopping-4.js" :lines "26-43" src javascript -n
...
We define two variables in the global scope.

Line 4 also introduces a curly bracket, so everything from there on in
is at L1 scope.

Line 5 uses the =inputBox= variable. We're in L1 scope and the
variable was defined in global, so this is fine.

Line 8 has a closing curly brace, so we jump up one scope level, and
we're back to global scope.

On line 10 I can use the =inputBox= variable instead of calling
=querySelector= again.

Lines 10 through 15 are very similar. Importantly, when the L1 scope
is created in line 10, and the L2 scope in line 11, these are
*different* from the scopes from earlier.
...
#+INCLUDE: "samples/shopping-4.js" :lines "26-47" src javascript -n
...
#+INCLUDE: "samples/shopping-5.js" :lines "26-47" src javascript -n
...
#+INCLUDE: "samples/shopping-6.js" :lines "26-47" src javascript -n
...
#+INCLUDE: "samples/shopping-7.js" :lines "26-29" src javascript -n 26
...
#+INCLUDE: "samples/shopping-7.js" :lines "39-52" src javascript -n 40
...
#+INCLUDE: "samples/shopping-7.js" :lines "30-38" src javascript -n 31
...
#+INCLUDE: "samples/shopping-8.js" :lines "7-28" src javascript

*** Replacing =let= with =const=

#+INCLUDE: "samples/shopping-8.js" :lines "29-32" src javascript

* Refactoring: =else=                                              :noexport:
...
* Exercise: Keeping our code DRY

#+INCLUDE: "samples/shopping-8.js" :lines "42-55" src javascript -n 42

- Don't Repeat Yourself (DRY)

#+begin_notes
Generally, it's good coding practice to have code that doesn't do the
same thing twice. A common term for that is DRY code -- as in "Don't
repeat yourself".

Sometimes the way we avoid repetition is by putting common code in to
functions that we can call -- we might call the function multiple
times, and that's absolutely fine -- but we only need to write the
code once.

Sometimes, though, we get repetition even within a function. There's
two particular pieces of repetition I want to focus on here.

The first is where =inputbox.value.trim()= is called repeatedly. Now,
it's only done twice, which is not a huge problem in this case. This
is not a particulaly *expensive* method to call -- by which I mean it
runs very quickly -- so calling it twice isn't going to result in code
that is meaningfully slower.

However, if we can reduce duplication we should try to -- it can make
our code clearer and our intentions easier to understand.

The second piece of repetition is the two =if= statements. There are
two glaring red flags here which we'll go in to more detail shortly.
#+end_notes

** Exercise: Put duplicated code in to a variable

#+INCLUDE: "samples/shopping-8.js" :lines "42-55" src javascript -n 42

- Create a variable called =trimmedValue=
- It's value should be the result of calling =.trim()=
- Use =trimmedValue= in both =if= expressions

** Solution: Hoisting duplicate code in to a variable

 #+INCLUDE: "samples/shopping-9.js" :lines "40-55" src javascript -n 40

 #+BEGIN_NOTES
 Here I've created a new constant variable called =trimmedValue= and
 set it to the trimmed copy of whatever's in the input widget.
 ...
 Who didn't write code like this?
...
** Refactoring: =false= and =true= inside the =if= block
 ...
 - Assigning =true= or =false= inside an =if= block is a big red flag
 - Especially if there are paired =if= statements

 #+begin_notes
 Earlier I mentioned that the repetition of the two =if= statements
 was a big red flag for me, so lets walk through why that is.

 And to be really clear, I'm specifically talking about the code on
 lines 44 and 52.

 Lets look at why this.
 #+end_notes

** =true= and =false= expressions with =if=
 ...
 #+BEGIN_NOTES
 Remember the structure of an =if= block, here on the left.

 In particular, the code inside the parentheses immediately after the
 =if= keyword is an expression that must evaluate to either a true or
 a false value.

This can be any expression that evaluates to =true= or =false=. Like
the examples here on the right.
 #+END_NOTES
...
const isEmpty = trimmedValue === ''; // true or false
#+END_SRC

#+BEGIN_NOTES
This means we could write something like this. If the trimmed value is
the empty string then =isEmpty= will be true.

And if the trimmedValue is not empty then =isEmpty= will be false.
...
#+begin_notes
Here's what this looks like if we throw in some sample values.

The first column represents the value that the user has typed in. The
second column is the trimmed version of that value, and the last
column is what the value of the =isEmpty= variable would be.

On the first row the user has typed in some text and there are some
spaces at the end. The trimmed version of that is in the second
column, and because this is not equal to the empty string =isEmpty= is
=false=.
...
The second row is very similar -- the user's typed in a string with
nothing to trim, so =isEmpty= is also =false=.

On rows three and four the user has typed in something that does trim
to the empty string, so in these cases =isEmpty= is =true=.
...
| =inputBox.value= | =.trim=   | =isEmpty= | =addItemButton.disabled= |
|------------------+-----------+-----------+------------------------|
| ='hello   '=     | ='hello'= | =false=   | ?                      |
| ='hello'=        | ='hello'= | =false=   | ?                      |
| ='    '=         | =''=      | =true=    | ?                      |
...
#+begin_notes
Lets extend the table and add a fourth column. This column contains
the value that =addItemButton.disabled= should have in each of these
scenarios.

So for the first row -- if the user has typed in the value in the
first column, what value should =addItemButton.disabled= have?
...
| =inputBox.value= | =.trim=   | =addItemButton.disabled= |
|------------------+-----------+--------------------------|
| ='hello   '=     | ='hello'= | =false=                  |
| ='hello'=        | ='hello'= | =false=                  |
| ='    '=         | =''=      | =true=                   |
...
#+begin_notes
Since this is exactly how we want the value for the =disabled=
property to be calculated there's no need to create an intermediate
variable, so we can use =trimmedValue= directly when assigning to the
=disabled= property.
#+end_notes
...
#+begin_notes
Just as a reminder, here's what our code currently looks like. Lets
change those hardcoded =true= and =false= values on lines 44 and 52 to
use the new assignment.
#+end_notes

** Refactoring: =false= and =true= inside the =if= block

#+INCLUDE: "samples/shopping-10.js" :lines "40-55" src javascript -n 40

#+begin_notes
Here's what that looks like.

Notice anything special about lines 44 and 52?

[Wait]

They're identical.

And since =if= expressions are exactly the opposite of each other --
if we don't do one of them then we definitely take the other -- we
know that line 44 or line 52 will *always* be run.

Since they'll always be run we can put those lines outside of the =if=
block entirely.

This will make the body of the second =if= block be empty, so we can
delete it too.
#+end_notes

** Refactoring: =false= and =true= inside the =if= block

#+INCLUDE: "samples/shopping-11.js" :lines "40-51" src javascript -n 40

#+begin_notes
Here's what that looks like.

On line 42 we *always* set the button's =disabled= property to either
=true= or =false= depending on whether or not the trimmed value is the
empty string.

After that is the =if= statement to see if the user has pressed the
=Enter= key.

This code does exactly the same thing as the previous code, but it's a
little but shorter and a little bit clearer.
#+end_notes

* Exercise: Refactoring with the =else= keyword           :exercise:noexport:

#+INCLUDE: "samples/shopping-7.js" :lines "40-53" src javascript -n 40
...
#+INCLUDE: "samples/shopping-9.js" :lines "40-51" src javascript -n
...
#+INCLUDE: "samples/shopping-12.js" :lines "40-52" src javascript -n 40
...
#+INCLUDE: "samples/shopping-13.js" :lines "44-60" src javascript -n 40
...
#+INCLUDE: "samples/shopping-13.js" :lines "35-43" src javascript -n 31
...
Notice how this code does not need to check to see if =trimmedValue= is empty. Why is that?

[Wait]

We have to think about how the user can interact with the button.

Remember that the button can either be disabled or enabled.

It starts off being disabled, so the user can't click on it. This means our code here will never run when the button is disabled.

And when does the button become enabled?

It's enabled in the =keyup= listener. In that code we check to see if the =trimmedValue= is the empty string. If it is we never enable the button.

This means we can be certain that the click handler will only run *if* the user has entered a valid value. Because if they haven't then the button won't be enabled, and the click handler won't run.
...
#+INCLUDE: "samples/shopping-7.js" :lines "26-54" src javascript

#+REVEAL: split

#+INCLUDE: "samples/shopping-13.js" :lines "31-66" src javascript
...
#+INCLUDE: "samples/shopping-15.html" :lines "14-21" src html
...
#+INCLUDE: "samples/shopping-15.js" :lines "32-37" src javascript

#+INCLUDE: "samples/shopping-15.js" :lines "69-77" src javascript
...
#+INCLUDE: "samples/shopping-15.js" :lines "37-46" src javascript

*** Solution: Enable the button when items are added

#+INCLUDE: "samples/shopping-15.js" :lines "47-63" src javascript
...
#+INCLUDE: "samples/shopping-15.js" :lines "73-76" src javascript
...
#+INCLUDE: "samples/shopping-15.js" :lines "19-25" src javascript
...
*** =while= loop to delete elements
 ...
 #+BEGIN_NOTES
 Here's how we could delete elements with a =while= loop, just as
 a reminder.
 #+END_NOTES
...
* Speed, and =document.readyState=                                 :noexport:
...
- This is an *optional* value
  - It's OK if the user doesn't type anything in
- Quantity should be added to the item description in the list

** New DOM fragment

#+REVEAL_HTML: <div class="leftcol">
#+BEGIN_SRC html
<li><span>Eggs</span> <span>(6)<span>
  <button>Delete</button></li>
#+END_SRC
#+REVEAL_HTML: </div>

#+REVEAL_HTML: <div class="rightcol">
#+BEGIN_SRC dot :file list-item-quantity-graph.svg :cmdline -Tsvg -Gstylesheet=../../graphviz.css
digraph G {
  node [shape="hexagon"];
  ...
  span1 [label="span"];
  span1 -> span1_content;
  span1_content [label="Eggs" shape=none];
  space1 [label="' '" shape=none];
  span2 [label="span"];
  span2 -> span2_content;
  span2_content [label="(6)" shape=none];
  space2 [label="' '" shape="none"];
  button -> button_content;
  button_content [label="Delete" shape=none];
}
...
#+RESULTS:
[[file:list-item-quantity-graph.svg]]
#+REVEAL_HTML: </div>
...
** Extract the value of the quantity widget

- In the same way we extract the value of the item widget
...
** Solution: HTML changes

#+BEGIN_SRC html
<input type="text" id="quantity" placeholder="Quantity">
#+END_SRC

** Solution: Extract the value of the quantity widget

#+INCLUDE: "samples/shopping-19.js" :lines "45-50" src javascript

#+BEGIN_NOTES
Here's the code that finds the quantity widget.
#+END_NOTES

** Solution: Extract the value of the quantity widget

#+INCLUDE: "samples/shopping-19.js" :lines "51-66" src javascript -n

#+begin_notes
In the click listener I pass the trimmed value to
=createNewListItem=. I don't bother saving the result in to a
variable, since I don't need to test the value -- it's OK to want to
add an item to the list without it having a quantity.

Notice that I also set the focus back to the quantity widget when the
item has been added.

This is because in my UI the quantity box comes first, so it makes
sense if the focus goes to it after an item has been added. If you've
put the quantity widget after the input widget in your UI then it
makes sense for the focus to go to the item widget instead.
#+end_notes

** Solution: Extract the value of the quantity widget

#+INCLUDE: "samples/shopping-19.js" :lines "67-86" src javascript -n

#+begin_notes
I've made the same change in the =keyup= handler as well.
#+end_notes

** Solution: Pass param to =createNewListItem=

#+INCLUDE: "samples/shopping-19.js" :lines "1-9" src javascript
...
#+INCLUDE: "samples/shopping-19.js" :lines "26-36" src javascript
...
And here, below, is the code that appends the delete button. These are exactly as before.

The new code is here in the middle.

We check to see if the quantity parameter is the empty string. If it is then there's nothing to do.

If it contains text then we create a text node with that text and append it, and then create a span for the quantity value. The span's =innerText= property is updated to display the information, and then it's added to the DOM as normal.
#+END_NOTES

** Problems?

#+BEGIN_NOTES
What problems do you see with the UI at this point?
...
- Quantity is not cleared when adding items
  - How do you fix this?
- Pressing =Enter= in quantity widget doesn't add the item

#+BEGIN_NOTES
Here are some problems that I saw when I was trying this.

1. The default width for an =input= widget is quite wide. How do you go about fixing this?

2. The focus might be wrong -- I put my "quantity" widget first, so it should get the focus first instead of the item widget.

3. The value in the quantity widget is not cleared after adding an item.

This we know how to fix, it's just a case of putting the right code in the right place...

4. Pressing the =Enter= key in the quantity widget doesn't do anything

[Have them go and do that]
#+END_NOTES
...
#+INCLUDE: "samples/shopping-19.html" :lines "17-19" src html
...
- Problem: Our =keyup= handler only listens for =inputBox= events
- =quantityBox= is a different element
- Three possible solutions

**** Solution 1: Duplicate the event handler

#+BEGIN_SRC javascript
inputBox.addEventListener('keyup', function (event) {
  // ... code that handles the click
});

quantityBox.addEventListener('keyup', function(event) {
  // ... Copy of the code from the other listener
});
#+END_SRC

#+BEGIN_NOTES
We could just -- literally -- copy and paste the code from the first input handler, and add it as an event listener for the =quantityBox=.

This will work, and it can be a good first step just to ensure that this fixes the problem.

But it's a terrible long term solution, as any time we have significant amounts of repeated code it indicates a problem with our design.
#+END_NOTES

**** Solution 2: Move the event handler in to a top-level function
...
Here, we take the keyup handler code and move it in to its own function, separate from everything else.

We call this =onKeyup= -- it's a convention that if you have a function that's acting as an event handler (i.e., it takes an =event= parameter, and is called as an event handler) that you call it =on= plus the name of the event it handles.

Since this handles the =keyup= event we call it =onKeyup= (note the lowerCamelCase name).
#+END_NOTES

** Solution 3p: Event bubbling                                :noexport:

- What if we added more =input= elements?
  - We'd have to duplicate this code again
- Can we avoid that?
- Yes!
...
#+BEGIN_NOTES
Here is a simplified version of the HTML for our app. I've omitted the =head= and the stuff that deals with the =ul= element.

Focus on the =div= that contains the =label=, the =input= elements, and the =button= elements for the moment.
#+END_NOTES

*** Events "bubble" up

#+BEGIN_SRC dot :file bubble-dom.svg :cmdline -Tsvg -Gstylesheet=../../graphviz.css
  digraph G {
    node [shape="hexagon" fontname="Consolas" fontsize=16];

    html -> body -> div;
    div -> label;
    input_item [label="input id=\"item\""];
    input_quantity [label="input id=\"quantity\""];

    div -> input_item;
    div -> input_quantity;

    button_add [label="button id=\"add\""];
    button_clear [label="button id=\"clear\""];
...
#+BEGIN_NOTES
Here is the DOM tree for that part of page.

Notice that these elements are all children of the =div=.

How does this help us with events?

Lets look at what happens at the moment when the user releases a key in the =input= element with the =id= set to =item= (the second element from the left in the bottom row of the tree).

The event is sent to the input element. The browser checks to see if there is an event listener for this event, and there is -- the one we added.

So the browser calls our =onKeyup= function, which adds the item to the list.

However, the browser does not stop there. Instead, it also checks to see if there is a =keyup= event handler on the =div=. There isn't, so nothing happens.

But it continues to check up the DOM tree. So it looks to see if there is a =keyup= event handler on the =body=, and on the =html= element.

Again, there aren't. But it checks anyway.

We say that the event "bubbles" up the DOM tree. Think of a bubble in a fizzy drink. It starts at the bottom, and floats up to the top of the glass. Events in the DOM are the same -- the start at element that they originally happened for, and then they "bubble" up the DOM tree until they reach the very top.

Every event handler in the path of the "bubble" that's listening for this type of event gets called along the way.

So -- imagine what happens if, instead of having two event handlers, one for each =input= element, we have one event handler that's listening on the =div=.

In this scenario, the user releases a key in an =input= element. The =keyup= event fires, but there's no listener on the element. The event bubbles up the tree, and then fires on the =div=. This time there is an event listener, so that gets called, and can process the event.

The event continues to bubble up the tree, but there are no handlers on the =body= or =html= element, so nothing more happens.

There is a problem with this approach though -- specifically, the =input= elements are not the only element that can fire the =keyup= event.

If a =button= has the focus then the user can press the =Space= bar. This has the same effect as clicking on the button, but it also fires the =keyup= event.

We don't want to respond to this, we only want to respond to =keyup= events that were caused from the =input= elements. So how do we process only events that came from the =input= elements?

Remember that our event handlers all receive a parameter that contains information about the event. This parameter is an object with many different properties. We use this in the =keyup= handler, by checking to see if =event.key= is =Enter= and deciding whether to add the item or not.

One of the other properties that the =event= contains is called =target=. This event contains a reference to the element on which the event originally fired.

So if our event handler is listening on the =div=, and an event bubbles up that originally came from one of its child elements, the =target= property  will refer to the exact element that the event was originally fired for.

The value in the =target= property is an =Element=, which means that it has a =nodeName= property that contains the element's name, for example =INPUT=, or =LABEL=, or =BUTTON=.

So we can change our =keyup= handler slightly to check to see if =event.target.nodeName= is =INPUT= -- if it is then this event originally came from one of the =input= elements, and should be processed. Otherwise, we should ignore the event.
#+END_NOTES

*** Event handler with bubbling

- We can remove the =onKeyup= function
  - And the =inputBox= / =quantityBox= event listeners
- Create an event listener on the =div=
- Check =event.target.nodeName=
  - Return early if not =INPUT=
...
function domContentLoaded() {
  ...
  div.addEventListener('keyup', function(event) {
    ...
    const trimedValue = inputBox.value.trim();
...
* TODO Bubbling and deleting list items                            :noexport:

#+BEGIN_NOTES
Note to self: This is incomplete. The code (see shopping-19-4.js) doesn't work quite as intended, because if you have e.g., <button><img...></button> it's likely that the click hits the img element and not the button element, and the test doesn't work. Need to see if I can come up with a more elegant way of explaining that / solving the code issue]

We can use this new knowledge to also simplify how we delete items from the list.

At the moment, each list item gets a delete button, and we attach a handler to that button to remove the element from the list.

This is OK, but it makes our =createNewListItem= function a bit more complicated than it needs to be.

But know we know about event bubbling we can simplify them.
#+END_NOTES
...
#+BEGIN_SRC html
  <ul>
    <li><span>Eggs</span> <span>6</span> <button>Delete</button></li>
    <li><span>Milk</span> <span>1L</span> <button>Delete</button></li>
    <li><span>Flour</span> <span>500g</span> <button>Delete</button><li>
  </ul>
#+END_SRC

#+BEGIN_NOTES
Here's what our shopping list looks like with three items.

I've ignored most of the page -- this is just the HTML for the list and the items.
#+END_NOTES

** DOM of list with three items

#+BEGIN_SRC dot :file bubble-dom-ul.svg :cmdline -Tsvg -Gstylesheet=../../graphviz.css
  digraph G {
    node [shape="hexagon"];
    ...
    li1 [label="li"];
    nameSpan1 [label="span"];
    quantitySpan1 [label="span"];
    delete1 [label="button"]
    li1 -> nameSpan1;
    li1 -> quantitySpan1;
    li1 -> delete1;
    nameSpan1 -> eggs;
    ...
    eggs [shape=none label="Eggs"];
    six [shape=none label="6"];
    delete1Text [shape=none label="Delete"];

    li2 [label="li"];
    nameSpan2 [label="span"];
    quantitySpan2 [label="span"];
    delete2 [label="button"]
    li2 -> nameSpan2;
    li2 -> quantitySpan2;
    li2 -> delete2;
    ...
    milk [shape=none label="Milk"];
    litre [shape=none label="1L"];
    delete2Text [shape=none label="Delete"];

    li3 [label="li"];
    nameSpan3 [label="span"];
    quantitySpan3 [label="span"];
    delete3 [label="button"]
    li3 -> nameSpan3;
    li3 -> quantitySpan3;
    li3 -> delete3;
    nameSpan3 -> flour;
    ...
    flour [shape=none label="Flour"];
    500 [shape=none label="500g"];
    delete3Text [shape=none label="Delete"];
  }
#+END_SRC

#+RESULTS:
[[file:bubble-dom-ul.svg]]

#+BEGIN_NOTES
And here's the DOM tree.

Right now, every delete =button= on that page has an event listener attached, waiting for the click event to fire. When it's fired, it removes the =li= element, which removes everything below it.

However, we could do this differently.

We could listen for click events on the =ul= element.

When the event fires, we can use the =target= property to find the element that was clicked.

If it wasn't a button then we ignore the click.

If it was a button then we know it was a delete button. If we know a delete button was clicked, how do we find out which =li= to remove?
#+END_NOTES

** =element.parentElement=

- Each element has a =parentElement= property
- Returns the parent element for this node in the DOM
- For our buttons, =parentNode= will return the =li= element

#+BEGIN_NOTES
Each element has various properties and methods available to it.

We're already familiar with some of them -- for example, the =remove()= method, or the =localName= property.

One of the properties we haven't seen yet is the =parentElement= property.

As the name implies, this property contains the element that is the parent of this one, in the DOM tree (or =null=, if we're looking at the root node).

So, if we've found a button then its parent element  is the =li= that it's associated with.

The =parentElement= property is the complete element for the parent. Which means it has a =remove()= method that will remove it, and all of its children.
...
person.name = {first: 'Nik', last: 'Clayton'};
#+END_SRC
...
#+INCLUDE: "samples/shopping-20.js" :lines "1-8" src javascript
...
#+INCLUDE: "samples/shopping-20.js" :lines "10-12" src javascript

#+INCLUDE: "samples/shopping-20.js" :lines "27-33" src javascript
...
- 4 differences from regular functions
  ...
  - Always returns the object being constructed
...
And you don't need to =return= a value explicitly, the new object is always returned for you.
...
*** Setup
 ...
 - Create =scratch/objects= directory
 - Create =scratch/objects/index.html=
 - Create =scratch/objects/objects.js=
 - Link them together with =<script>=
...
#+INCLUDE: "samples/shopping-21.js" :lines "39-50" src javascript
...
#+INCLUDE: "samples/shopping-21.js" :lines "62-72" src javascript -n 62
...
#+INCLUDE: "samples/shopping-21.js" :lines "79-94" src javascript -n 79
...
#+INCLUDE: "samples/shopping-21.js" :lines "1-8" src javascript
...
#+INCLUDE: "samples/shopping-22.js" :lines "13-19" src javascript
...
#+INCLUDE: "samples/shopping-22.js" :lines "21-23" src javascript

#+INCLUDE: "samples/shopping-22.js" :lines "38-44" src javascript
...
#+INCLUDE: "samples/shopping-22.js" :lines "62-78" src javascript
...
#+INCLUDE: "samples/shopping-22.js" :lines "79-100" src javascript
...
- Update your code
  - Replace current constructor with new style
  - Replace =toListItem= with new style
...
- =shopping.js= is starting to get large
- Also has multiple responsibilites
  - Code for =ShoppingListItem=
  - Code for the user interface
- Good idea to split in to two

** Split in to two files

- =shopping-list-item.js=
  - Put =class ShoppingListItem { ... }= in here
- =ui.js=
  - Everything else in here
- Update the HTML

#+INCLUDE: "samples/shopping-multiple/shopping.html" :lines "27-29" src html

** Problem

- Each file the browser has to load slows things down
- We want things to be fast
- Tension
  - Multiple files -- easier for us
  - Single file -- better for the browser
- We'll fix this later on

#+BEGIN_NOTES
We might not realise it yet, but we've just introduced a problem in to the site.

Every extra file that the browser needs to load will slow loading our site down a little bit. There's an unavoidable delay that happens when the browser needs to start download and processing a new file. It's small, but it's there, and it adds up. If we had a larger application with lots of files this would become a real problem.

This penalty is the same irrespective of how large the file is.

So there's a tension here between doing something that makes things easier for us -- having small files -- and doing something that makes things better for the browser.

Later we'll look at how to fix that so that we get the best of both worlds -- sort of. For the moment I just want you to be aware that this is a problem.
#+END_NOTES
...
** Model - View - Controller                                       :noexport:

- Not exporting until I get the diagram right

#+BEGIN_SRC dot :file mvc.svg :cmdline -Tsvg -Gstylesheet=../../graphviz.css
  digraph G {
    edge [fontsize=18];
    ...
    model -> view [xlabel="Updates" tailport=w headport=n];
    controller -> model [label="Manipulates" headport=e tailport=n];
    view -> controller [label="Informs" tailport=s headport=s];
...
- =Model= contains data, and methods to change it
  - When data changes, tells the =View= to update.
...
- Create directories
  - =shopping-mvc/=
  - =shopping-mvc/src/=
  - =shopping-mvc/src/css/=
  - =shopping-mvc/src/html/=
  - =shopping-mvc/src/js/=
  - =shopping-mvc/src/static/=

#+BEGIN_NOTES
We're going to start structuring the code in our application more like how you would see it in a real app.

There's no one correct way to do this -- different organisations or projects will have different standards.

Here we're creating a top-level directory, =shopping-mvc=, and a =src= directory inside it. This is, in turn, going to have directories for the different parts of the source code that make up our app.

As I say, this will work for us, but you will definitely see other ways to organise code in your future projects.
#+END_NOTES

** Copy over files from =shopping-class/=

- =*.js= into =js/=
- =shopping.html= into =html/=
- =*.css= into =css/=
- Images into =static/=
- Update paths in the files
  - E.g., to reference an image from a CSS file
    - =../static/name-of-image.png=

** Skeleton code

#+BEGIN_NOTES
We're going to start by putting in place the bare minimum code for our app.

It's not going to do anything useful yet, but it is going to show the structure, and wire together everything for us.
#+END_NOTES
...
#+INCLUDE: "samples/shopping-mvc/skeleton/src/js/controller.js" src javascript

*** Model

- Creates the =items_= array, saves the view.
- =src/js/model.js=

#+INCLUDE: "samples/shopping-mvc/skeleton/src/js/model.js" src javascript

*** View

- Save the controller in to a property
- =src/js/view.js=

#+INCLUDE: "samples/shopping-mvc/skeleton/src/js/view.js" src javascript

*** Initialise the application

- =src/js/ui.js=

#+INCLUDE: "samples/shopping-mvc/skeleton/src/js/ui.js" src javascript

#+REVEAL: split

#+INCLUDE: "samples/shopping-mvc/skeleton/src/html/index.html" :lines "27-32" src html

*** Control flow

- Page is loaded
  - =ui.js= is run, =new Controller()=
  - =controller.js:constructor()= is run, =new Model(this)=
  - =model.js:constructor()= is run, =new View(this, controller)=
  - =view.js:constructor()= is run
- Constructed a lot of objects
...
- View:
  ...
  - Will need to receive the items as a paramater
- Model:
  - Create some dummy data
  - Call the View's update method to tell the view to update
    - Gives the view the data
    - Prevent changes to the =items_= array

*** Exercise: =View.update(items)=

- Given a parameter, =items=
- Find the shopping list element (=ul=)
- Remove all its children
- Iterate over the =items= elements
  - Convert each one to a listitem (=li=, use the =toListItem= method)
  - Append it as a child to the list

*** Solution: =View.update(items)=

#+INCLUDE: "samples/shopping-mvc/dummy-data/src/js/view.js" :lines "8-15" src javascript

#+REVEAL: split

#+INCLUDE: "samples/shopping-mvc/dummy-data/src/js/view.js" :lines "16-32" src javascript
...
#+INCLUDE: "samples/shopping-class-3.js" :lines "26-27" src javascript

#+INCLUDE: "samples/shopping-class-4.js" :lines "38-43" src javascript

*** Tell the view to update

- =View.update()= needs to be called when the model's =items_= array changes

#+BEGIN_SRC javascript
//
this.view_.update(this.items_);
//
#+END_SRC

**** Problems with this approach

- Parameter values  can be passed in two ways
  - As a copy
  - As a reference to the real thing
- Complex values are passed differently than simple values
- Simple values: Pass as a copy
- Complex values: Pass the real thing
  - Arrays (and objects) are complex values
...
#+BEGIN_NOTES
Let's look at how simple values and complex values behave differently.

In the example on the left we have a function that takes a single parameter, adds 10 to it, and returns it.

On line 6 we create a variable called =a= and give it a value. On the next line we create a new variable =b=, and set its value by calling =foo= passing =a= as the parameter.

Inside the function the parameter has the name =x=. The function changes the value of =x=, and then returns the value.

When we call =foo=, it receives a *copy* of the value in =a=. So when the =foo= function changes the value, it's changing the copy. The original value inside =a= is not changed.

We can see this by checking the value of =a= and =b= after the function has returned. As we can see, =a= has its original value, and =b= has the new value.

That's how simple values, like numbers and strings work.

This is super important, so I'll repeat it -- in the example on the left, the function receives a *copy* of the value in the variable.

When functions receive or return arrays or objects, they behave differently. In this example I've used an array, but objects are the same.

Instead of receiving a *copy* of the array, the function on the right receives the actual array.

So if the function changes that array in any way, the original array is modified as well.

In our function, on line 2, we use the =push= method to add the value =10= to the end of the array.

*This changes the original array*

On line 6 and 7 we do effectively the same thing as we did with the function on the left. However, we're doing this with arrays instead.

Line 6 creates the array with three values. Line 7 calls the function, and saves the result in a new variable.

However, when we print the values in the variables, on lines 9 and 10, we see that =a= has been modified.

Compare this with the example on the left, also line 9 -- there, the value in =a= was not changed.

This is why you have to be careful with functions that handle arrays and objects. If you're not careful they can end up making changes that you don't want.

It gets worse.

By returning the array from the =bar= function, our =a= and =b= variables now refer to the *same* array. If we change one of them, we change the other.

On line 12 we push a value on to the array in =a=. But because =a= and =b= both refer to the same array, this also changes the value in =b=.
#+END_NOTES

**** Changing how we think about variables
...
#+BEGIN_NOTES
This means we should change how we think about variables.

So far we've been thinking about variables as being a box with a label on the side and a value inside them.

And arrays are a collection of boxes, where we refer to each box by it's main name, and then the number.

One box (containing the value), and one label (the variable name).

However, what we've seen in this example is that we can we have two variables that refer to the same variable.

So the way to think about this is that our box that contains the value has two (or more) labels on it. You can refer to the value using any of the labels on the side of the box.
#+END_NOTES

**** Why is this a problem?

#+REVEAL_HTML: <div class="leftcol">
- Model is responsible for the data
- All changes to the data should go through the model
- But this breaks that
#+REVEAL_HTML: </div>
...
#+BEGIN_NOTES
On the left is our hypothetical code in the Model. It ceates an array and assigns it to =items_=.

Then it calls the =update()= method.

However, it doesn't return a copy of the array, it returns the actual array.

On the right is code that shows how this breaks.

Here we assume that our =update= method decides to change the items in some way. Now, our code won't do that, but it's still a risk. Because now we have changed the original array.

*We should not do this* -- all changes to data managed by the model should be made by calling methods that the model provides.
#+END_NOTES

**** Solution: Model provides a copy of =items_=

- =Model= needs to provide a *copy* of the =items_= array
- How?

#+BEGIN_NOTES
In order to solve this, the =Model= needs to provide a copy of the =items_= array. Then any code that uses it can modify the copy, and the original in the Model will be unchanged.
...
**** Solution: =Array.prototype.slice()=

- Arrays have a =slice= method
- Returns a copy of some or all of an array

**** =Array.prototype.slice()= examples

#+BEGIN_SRC javascript
let cities = ['Basel', 'Bern', 'Lausanne', 'Zurich'];

let b = cities.slice(0, 2);
b;                 // ['Basel', 'Bern'];
b.push('London');
b;                 // ['Basel', 'Bern', 'London']
cities;            // ['Basel', 'Bern', 'Lausanne', 'Zurich'];
#+END_SRC

#+BEGIN_NOTES
In this example we create an array called =cities= with four string values.

Then we create a new array, =b=, by taking a *slice* of =cities=. The slice starts at element index 0, up to, but not including, the element at index 2. So this takes the elements at index 0, and 1 -- the first two elements.

This slice in =b= is *copy* of what was in =cities=. So if we modify =b= by pushing values on to it, as we do here, the values in the =cities= array are not changed.
...
let c = cities.slice();  // Complete copy of cities
c;                 // ['Basel', 'Bern', 'Lausanne', 'Zurich']
c.push('London');
c;                 // ['Basel', 'Bern', 'Lausanne', 'Zurich', 'London'];
cities;            // ['Basel', 'Bern', 'Lausanne', 'Zurich'];
#+END_SRC

#+BEGIN_NOTES
If we call =slice= with no parameters then we get back a complete copy of the array. Because this is a copy we can modify it and the original array is not changed.
#+END_NOTES
...
*** Create a method to retrieve the =items_= array                 :noexport:

- The =View= needs to retrieve data from the model
- The model stores the data in the array =items_=
- Model should provide a method that returns this array
  - We'll call this method =items()=
...
- Complex values are returned differently than simple values
- Simple values: Return a copy
- Complex values: Return the real thing
  - Arrays (and objects) are complex values
...
Then the =items()= method returns this array.

However, it doesn't return a copy of the array, it returns the actual array.

On the right is code that shows how this breaks.

On the first line we create a new model object, and save it in to a variable. Then we call the =items()= method and save the result in to a variable as well.

Then we call the =push= method on this array. But now we haven't just changed our copy of the items, we've changed the master copy in the model as well.

*We should not do this* -- all changes to data managed by the model should be made by calling methods that the model provides.
#+END_NOTES

**** Solution: Model returns a copy of =items_=

- =Model= needs to return a *copy* of the =items_= array
- How?

#+BEGIN_NOTES
In order to solve this, the =Model= needs to return a copy of the =items_= array. Then any code that uses it can modify the copy, and the original in the Model will be unchanged.
...
*** Provide a =getter= for the =items_= array                      :noexport:

- A what?

*** Getter turns a method in to a property                         :noexport:
...
#+BEGIN_SRC javascript
// In View
let length = this.model_.items().length;
#+END_SRC
...
*** Exercise: =Model= needs to call View's =update=

- The display won't update until the =Model= tells the =View= to update
- Model's constructor should call =update= after creating the view

*** Solution: =Model= needs to call View's =update=

#+INCLUDE: "samples/shopping-mvc/dummy-data/src/js/model.js" :lines "8-21" src javascript

*** Recap: Updating the display

- =Model= contains some dummy data
- =View= has an =update()= method that:
  - Deletes existing list
  - Creates new list using content from the model
- =Model= constructor updates the =View=
  - =this.view_.update()=

** Adding items

- User types in to input widgets, clicks button
- =View=
  - Receives the click, calls method in =Controller=
  - =update()= method resets values, focus
- =Controller=:
  - Performs any checks
  - Creates a new =ShoppingListItem=
  - Calls method in =Model= to add item
- =Model=
  - Adds item to internal array
  - Calls =View.update()=

*** =View=

#+INCLUDE: "samples/shopping-mvc/add-item-button/src/js/view.js" :lines "8-26" src javascript

#+REVEAL: split

#+INCLUDE: "samples/shopping-mvc/add-item-button/src/js/view.js" :lines "27-36" src javascript

#+REVEAL: split

#+INCLUDE: "samples/shopping-mvc/add-item-button/src/js/view.js" :lines "42-57" src javascript

*** =Controller=

#+INCLUDE: "samples/shopping-mvc/add-item-button/src/js/controller.js" :lines "13-23" src javascript

*** =Model=

#+INCLUDE: "samples/shopping-mvc/add-item-button/src/js/model.js" :lines "18-27" src javascript

- Don't forget to remove the dummy data!

** Deleting items

*** Changing how we delete items

- Previously
  - Set event listener when creating the list item
  - Mixed code responsibilities
    - =ShoppingListItem= object shouldn't know how to delete itself

#+REVEAL: split

- Now:
  - =View= creates list item
    - Will find the delete button
    - Forwards delete requests to the =Controller=
  - =Controller= forwards request to the =Model=
  - =Model= deletes the item from =items_=
    - Tells the =View= to update

*** =ShoppingListItem.toListItem()=

- Remove the code that adds the =click= listener
- We're going to listen in the =View=

*** =View=

- When adding items, find the delete button...
  - ... add a click listener that calls the controller

#+INCLUDE: "samples/shopping-mvc/delete-item-button/src/js/view.js" :lines "41-56" src javascript

*** =Controller=

#+INCLUDE: "samples/shopping-mvc/delete-item-button/src/js/controller.js" :lines "21-29" src javascript

*** =Model=

#+INCLUDE: "samples/shopping-mvc/delete-item-button/src/js/model.js" :lines "35-44" src javascript

** Clearing the list

- =View=
  - Listen for clicks, call =Controller.clearList()=
  - Maybe disable the button in =update()=
- =Controller=
  - Implement =clearList()=, call =Model.clear()=
- =Model=
  - Implement =clear=
  - Empty the list
  - Update the =View=
...
#+INCLUDE: "samples/shopping-mvc/clear-list-button/src/js/view.js" :lines "28-33" src javascript

- In =update=

#+INCLUDE: "samples/shopping-mvc/clear-list-button/src/js/view.js" :lines "61-65" src javascript

*** =Controller=

#+INCLUDE: "samples/shopping-mvc/clear-list-button/src/js/controller.js" :lines "30-36" src javascript

*** =Model=

#+INCLUDE: "samples/shopping-mvc/clear-list-button/src/js/model.js" :lines "45-52" src javascript

** The =Enter= key

- =View=
  - Listen for =keyup= events on both =input= elements
  - In handler:
    - Maybe enable/disable =addItemButton_=
    - Maybe call =addItem()=
  - Disable =addItemButton= in =update()=
- =Controller=
  - No change
- =Model=
  - No change
...
#+INCLUDE: "samples/shopping-mvc/keyup/src/js/view.js" :lines "31-35" src javascript

#+REVEAL: split

#+INCLUDE: "samples/shopping-mvc/keyup/src/js/view.js" :lines "44-66" src javascript

#+REVEAL: split

#+INCLUDE: "samples/shopping-mvc/keyup/src/js/view.js" :lines "86-91" src javascript

** Stats and benefits

#+REVEAL_HTML: <div class="leftcol">
- =Model=
  - 4 methods, 1-2 lines long
- =View=
  - 3 methods, 4-20 lines long
- =Controller=
  - 3 methods, 1-2 lines long
#+REVEAL_HTML: </div>

#+REVEAL_HTML: <div class="rightcol">
- Only one method to update the display
- Only one method to respond to keypresses
...
* Parsing quantities                                               :noexport:
* Re-order items in the list                                       :noexport:
* Drag-drop items in the list                                      :noexport:
* Javascript syntax followup work                                  :noexport:
* Event listeners and =this=                                       :noexport:
* Advanced: Using validity check to prevent invalid input          :noexport:
** TODO: Problem
** Things to check
