...
#+INCLUDE: "samples/css.html" src html
...
#+INCLUDE: "samples/css-2.html" :lines "8-23" src html
...
#+BEGIN_SRC html-chrome :url (shell-quote-argument (expand-file-name "samples/css-2.html")) :flags --window-size=320,320 --force-device-scale-factor=2 :exports results
#+END_SRC

#+RESULTS[06b3beabdbb0350edd0615741030f0ee941c0d31]:
...
** TODO Selecting all elements of a particular type
 ...
 #+INCLUDE: "samples/style-3.css" src css

- Check, add, and commit
 #+REVEAL_HTML: </div>

 #+REVEAL_HTML: <div class="rightcol">
 #+BEGIN_SRC html-chrome :url (shell-quote-argument (expand-file-name "samples/css-3.html")) :flags --window-size=320,320 --force-device-scale-factor=2 :exports results
 #+END_SRC

 #+RESULTS[0c1663979173d5831efb0482277a230e4dfec2d2]:
 [[file:selecting-all-elements-of-a-particular-type.png]]
...
#+REVEAL_HTML: <div class="leftcol">
#+INCLUDE: "samples/style-4.css" src css

- Check, add, and commit
#+REVEAL_HTML: </div>

#+REVEAL_HTML: <div class="rightcol">
#+BEGIN_SRC html-chrome :url (shell-quote-argument (expand-file-name "samples/css-4.html")) :flags --window-size=320,320 --force-device-scale-factor=2 :exports results
#+END_SRC

#+RESULTS[46807090cce5f251f11ae85c1e78d068bf00299a]:
[[file:selecting-one-element-using-id.png]]
...
#+REVEAL_HTML: <div class="leftcol">
#+INCLUDE: "samples/style-5.css" src css

- Check, add, and commit
#+REVEAL_HTML: </div>

#+REVEAL_HTML: <div class="rightcol">
#+BEGIN_SRC html-chrome :url (shell-quote-argument (expand-file-name "samples/css-5.html")) :flags --window-size=320,320 --force-device-scale-factor=2 :exports results
#+END_SRC
...
#+REVEAL_HTML: <div class="leftcol">
#+INCLUDE: "samples/style-6.css" src css

- Check, add, and commit
#+REVEAL_HTML: </div>

#+REVEAL_HTML: <div class="rightcol">
#+BEGIN_SRC html-chrome :url (shell-quote-argument (expand-file-name "samples/css-6.html")) :flags --window-size=320,320 --force-device-scale-factor=2 :exports results
#+END_SRC
...
#+REVEAL_HTML: <div class="leftcol">
#+INCLUDE: "samples/style-7.css" src css
#+REVEAL_HTML: </div>

#+REVEAL_HTML: <div class="rightcol">
#+BEGIN_SRC html-chrome :url (shell-quote-argument (expand-file-name "samples/css-7.html")) :flags --window-size=320,320 --force-device-scale-factor=2 :exports results
#+END_SRC
...
- Use CSS selectors to select items
- Around level 9 introduces new selectors to learn
- Do all 32 levels
- Search for [mdn css selectors] for more documentation

#+begin_notes
CSS Diner is a game that helps you learn the different CSS selectors.

[Demo the UI, show how to solve the first two levels]
#+end_notes
...
[This might be a good point for a break]
#+END_NOTES
...
#+BEGIN_SRC html-chrome :url (shell-quote-argument (expand-file-name "samples/colour-table.html")) :flags --window-size=1024,800 --force-device-scale-factor=1.0 :exports results
#+END_SRC
...
#+REVEAL_HTML: <div class="leftcol">
- 1 or 2 primary colours and variants
- 2 to 4 accent colours and variants
- Greys
#+REVEAL_HTML: </div>

#+REVEAL_HTML: <div class="rightcol">
[[file:samples/whats-in-a-color-palette-03.png]]
#+REVEAL_HTML: </div>

#+BEGIN_NOTES
This isn't a course that's going to turn you in to a web designer
over night.

But there are a few simple things you can think about when picking
colours for your site that will help it feel harmonious and pleasant
to look at.

Typically you will have one or two primary colours for your site.
This sets the overall tone for the site.

Once you've chosen the primary colour you will probably want to use
one or two variants of that colour, one darker, one lighter, to use
for emphasis and on certain UI elements.

As well as your primary colour you will typically have two to four
accent colours. These complement your primary colour, but are
different to it, and contrast well with it.

Then you'll have greys -- for text, backgrounds, some UI elements,
and so on.

Part of the art of web design is coming up with a harmonious colour
palette.
...
#+REVEAL_HTML: <div class="leftcol">
- [[http://paletton.com/][Paletton]]
- [[http://colorsafe.co/][Color Safe]]
- [[https://color.adobe.com/create/color-wheel/][Adobe Color CC]]
- [[https://flatuicolors.com/][Flat UI Colors]]
#+REVEAL_HTML: </div>

#+REVEAL_HTML: <div class="rightcol">
- [[http://www.pictaculous.com/][Pictaculous]]
- [[https://material.io/collections/color/#][Google Color Collection]]
- [[https://mycolor.space/][My Color Space]]
#+REVEAL_HTML: </div>

#+BEGIN_NOTES
There are many online tools to help you come up with a consistent
colour palette for your site.

A few of them are listed here.

Some of them ask you to pick one or two colours, and then it generates
the rest based on your choice.

Others, like Pictaculous, let you upload an image and it picks out
the dominant colours from the image and generates a palette based on
that.
#+END_NOTES
...
#+END_CENTER
#+BEGIN_SRC html
<p style="margin-left: 3em; color: rgb(255, 0, 0);">...</p>
#+END_SRC
#+REVEAL_HTML: </div>
...
**** Google Fonts example
 ...
 #+BEGIN_SRC html-chrome :noweb yes :cache yes :exports results :flags --window-size=400,240 --force-device-scale-factor=2
 <<google-fonts-example>>
 #+END_SRC

 #+RESULTS[980fc248203305c343462af4d95efe38d2d96133]:
...
*** Result: Google Fonts
 ...
 #+BEGIN_SRC html-chrome :exports results :flags --window-size=400,240 --force-device-scale-factor=2
   ...
   <html>
     <head>
       <title>Google Fonts example</title>
       <link href="https://fonts.googleapis.com/css?family=Cardo|Cutive+Mono|Titillium+Web" rel="stylesheet">
       <style>
	 h1, h2, h3, h4, h5, h6 {
	   font-family: 'Titilliam Web', sans-serif;
	 }
 ...
 #+RESULTS[6f0697bd6fb9b62b9a7592b740e09779b61fafa2]:
...
** TODO =font-size: ...=
 ...
 that the user agent stylesheet might change that.
...
** TODO =text-decoration: ...=
 ...
 p {
   text-decoration: line-through wavy rgb(255, 0, 0);
 }
 #+END_SRC
 #+REVEAL_HTML: </div>
...
#+BEGIN_NOTES
Now we're going to take a look at a very useful tool in web
development -- the tools that ship with the browser.

Today, most browsers include various tools that you can use to
inspect all aspects of the page that is being displayed. This can
be really helpful in understanding what's actually happening, and
if you're developing a page and it's not appearing the way that
you expected then the developer tools can help you figure out what's
wrong

So we're going to do a tour through some of the developer tools
available to you.

[Rest of notes is demo]
...
* TODO CSS Practice                                                :exercise:

#+REVEAL_HTML: <div class="leftcol">
- Style your recipes, cheatsheet, and portfolio
  - Different styles for each type
- Choose palette, fonts, general layout
- Enhance some content with different styles
  - No ransom notes!
...
#+BEGIN_NOTES
Earlier this week you started working on cheatsheets, recipes, and
a portfolio.
...
So I want you to amend those now and create CSS files for them.

Instead of trying to change all the styles at once I suggest that you
try and change styles in roughly the order:
...
This is pretty much the order of the impact that each decision will
have on the look of the page.
...
Your recipes should all share a stylesheet, so the style needs to
look good for all of them. The same for your cheatsheets -- they
share a stylesheet that's different from the one for the recipes.
And the portfolio should have a stylesheet for itself.

So three different stylesheets in total.

This 'web design in 4 minutes' link here walks through some of the
big impact that simple style changes can have, and I recommend that
you run through it quickly.
#+END_NOTES
...
- [[https://cs.chromium.org/chromium/src/third_party/blink/renderer/core/html/resources/html.css][Chrome's default stylesheet]]
- Argggggh!
...
<body>
  <p id="tip" class="info">Here is a paragraph</p>
</body>
#+END_SRC

#+RESULTS[976a23d371e19615d5c2977178a837ccf2fb2056]: css-specificity-3
...
#+begin_notes
By this point (teacher) should have seen the HTML and CSS produced by the students and they've had a chance to practice. This can be a good spot to review common problems and emphasise what is considered to be correct (per the course's styleguide).
#+end_notes
