...
(defun my/remove-generated-files (&optional bfn)
  "Remove files that are generated by the export process.

This prevents them from cluttering 'git status'.

Optional BFN is the value that should be used instead of buffer-file-name
to determine the directory that contains the files to be removed."
  (interactive)
  ;; TODO(?): Update a gitignore file instead?
  ;; WIP
  (org-element-map (org-element-parse-buffer) 'src-block
    (lambda (src-block)
      (let ((language (org-element-property :language src-block))
	    (files '()))
  	(cond
  	 ((string= language "html-chrome")
	  ;; TODO: Would be better if there was an ob-html-chrome
	  ;; function that returned the generated filename from
	  ;; a SRC block.
	  (let ((name (org-element-property :name src-block)))
	    (when name
	      (add-to-list 'files (format "%s.png" name)))))
  	 ((string= language "dot")
	  ;; No way to grab the :file value directly, so first get
	  ;; all the parameters (which returns a string) then parse
	  ;; to an alist, then find the value of the :file key.
	  (let* ((params (org-element-property :parameters src-block))
		 (parsed (org-babel-parse-header-arguments params))
		 (file (alist-get :file parsed)))
	    (when file
	      (add-to-list 'files file)))))
	(dolist (filename files)
	  (let ((filepath (f-join
			   (f-dirname (or bfn buffer-file-name)) filename)))
	    (ignore-errors
