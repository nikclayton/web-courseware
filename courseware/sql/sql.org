...
#+BEGIN_NOTES
Here we've used the =DESC= keyword, so now we get the tracks sorted by duration in descending order, with the longest track first, then the next longest, and so on.
#+END_NOTES
...
Here, we find all the tracks we're interested in and sort them by duration order.

When we have rows that have the same duration, like =508= or =505= in this example, we then sort them by name, in ascending order.
...
If you look at the results you can see that they are now in name order. Since we don't have any songs that have the same name the duration sorting doesn't make any sense.
#+END_NOTES
...
#+BEGIN_NOTES
If we change the query to find songs with =iron= in the name we get a clearer view. Now all the songs called =Iron Maiden= are together, and within that they're sorted by duration, with the longest songs coming first.
#+END_NOTES
...
- ... group by the name ...
- ... ignore any where the count is < 3
- ... display and and name, most popular first
#+REVEAL_HTML: </div>
...
#+BEGIN_NOTES
That's interesting -- we have some tracks where the composer column is empty.
#+END_NOTES
