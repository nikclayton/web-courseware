...
Edit =emacs.el= and change the definitions for the entries:

- =my/poco-path=
  - Path to the checked out =courseware= directory.
- =:base-directory=
  - Path to the root of the repository
- =:publishing-directory=
  - Path that output will be published to
